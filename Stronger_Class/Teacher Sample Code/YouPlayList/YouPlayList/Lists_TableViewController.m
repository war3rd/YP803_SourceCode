...
{
    //從 YouTube 取得 playListID 播放清單的曲目(影片列表清單)
...
//        lists = [NSMutableArray arrayWithArray:[[responseObject objectForKey:@"feed"] objectForKey:@"entry"]];
        lists = [NSMutableArray arrayWithArray:[responseObject valueForKeyPath:@"feed.entry"]];
        [self DownloadNext:responseObject];
        [self.tableView reloadData];
        self.title = [NSString stringWithFormat:@"%d", [lists count]];
        
//加上 tableHeaderView
/*
        NSString *imageString = [[[responseObject valueForKeyPath:@"feed.media$group.media$thumbnail"] objectAtIndex:1] objectForKey:@"url"];
        NSURL *url = [NSURL URLWithString:imageString];
        NSURLRequest *requestHead = [NSURLRequest requestWithURL:url];
        AFHTTPRequestOperation *operationHead = [[AFHTTPRequestOperation alloc] initWithRequest:requestHead];
        operation.responseSerializer = [AFImageResponseSerializer serializer];
        [operation setCompletionBlockWithSuccess:^(AFHTTPRequestOperation *operation, id responseObject) {
            
            dispatch_async(dispatch_get_main_queue(), ^{
                UIImageView *imgHead = [[UIImageView alloc] initWithImage:responseObject];
                self.tableView.tableHeaderView = imgHead;
            });
