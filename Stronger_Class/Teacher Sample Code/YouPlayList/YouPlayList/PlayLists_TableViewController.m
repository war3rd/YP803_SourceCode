...
//        playlists = [NSMutableArray arrayWithArray:[[responseObject objectForKey:@"feed"] objectForKey:@"entry"]];
        playlists = [NSMutableArray arrayWithArray:
                     [responseObject valueForKeyPath:@"feed.entry"]];
...
- (void)viewDidLoad
{
    ...
    self.title = @"binmusictaipei";
    [self getPlaylistsByID:@"binmusictaipei"];
}
...
//    UITableViewCell *cell = [tableView dequeueReusableCellWithIdentifier:CellIdentifier forIndexPath:indexPath];
    PListsTableViewCell *cell = [tableView dequeueReusableCellWithIdentifier:CellIdentifier forIndexPath:indexPath];
    
    // Configure the cell...
    ///TODO: 參考可以自動捲動 https://github.com/cbpowell/MarqueeLabel 或 https://github.com/cbess/AutoScrollLabel
    cell.textLabelP.text = [NSString stringWithFormat:@"%d:%@",
                            indexPath.row,
                            [[playlists objectAtIndex:indexPath.row] valueForKeyPath:@"title.$t"] ];
    cell.publishedP.text = [NSString stringWithFormat:@"%@",
                            [[playlists objectAtIndex:indexPath.row] valueForKeyPath:@"published.$t"] ];
    cell.countHint.text = [NSString stringWithFormat:@"%d",
...
//3.1 使用 UIImageView+AFNetworking category, 內定 TableViewCell
/*
    NSURL *url = [NSURL URLWithString:imageString];
    NSURLRequest *request = [NSURLRequest requestWithURL:url];
...
//3.2 使用 UIImageView+AFNetworking category, 使用 Custom TableViewCell
    NSURL *url = [NSURL URLWithString:imageString];
    NSURLRequest *request = [NSURLRequest requestWithURL:url];
    UIImage *placeholderImage = [UIImage imageNamed:@"fb001.png"];
    __weak PListsTableViewCell *weakCell = cell;
    [cell.imageView setImageWithURLRequest:request
                          placeholderImage:nil ///TODO: placeholderImage 無法取代, 先暫時設定 nil
                                   success:^(NSURLRequest *request, NSHTTPURLResponse *response, UIImage *image) {
                                       //
                                       weakCell.imageViewP.image = image;
                                       [weakCell setNeedsLayout];
                                   } failure:^(NSURLRequest *request, NSHTTPURLResponse *response, NSError *error) {
                                       //
                                   }];
...
- (IBAction)changeRowAction:(id)sender {
    NSArray *changeIndexPaths = @[[NSIndexPath indexPathForRow:0 inSection:0],
                                  [NSIndexPath indexPathForRow:2 inSection:0]];
    [self.tableView reloadRowsAtIndexPaths:changeIndexPaths withRowAnimation:UITableViewRowAnimationRight];
}
